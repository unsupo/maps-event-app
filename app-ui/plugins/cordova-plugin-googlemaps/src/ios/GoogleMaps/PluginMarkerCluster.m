...
- (void)create:(CDVInvokedUrlCommand*)command {
  ...
  [result setObject:clusterId forKey:@"eventId"];
...
- (void)redrawClusters:(CDVInvokedUrlCommand*)command {
  ...
  [self.mapCtrl.executeQueue addOperationWithBlock:^{
    ...
    for (int i = 0; i < new_or_updateCnt; i++) {
      ...
      clusterId_markerId = [clusterData objectForKey:@"eventId"];
      ...
      [properties setObject:clusterId_markerId forKey:@"eventId"];

      if ([clusterData objectForKey:@"icon"]) {
        eventId iconObj = [clusterData objectForKey:@"icon"];
    ...
    [[NSOperationQueue mainQueue] addOperationWithBlock: ^{
      ...
      for (int i = 0; i < [updateClusterIDs count]; i++) {
        ...
        if ([clusterId_markerId containsString:@"-marker_"]) {
          if (isNew) {
            [super _create:clusterId_markerId markerOptions:markerProperties callbackBlock:^(BOOL successed, eventId resultObj) {
        ...
        if (isNew) {
          // If the requested eventId is new location, create a marker
        ...
        if ([markerProperties objectForKey:@"icon"]) {
          ...
          [self setIconToClusterMarker:clusterId_markerId marker:marker iconProperty:icon callbackBlock:^(BOOL successed, eventId resultObj) {
            if (successed == NO) {
              //--------------------------------------
              // Could not read icon for some reason
              //--------------------------------------
...
- (void) setIconToClusterMarker:(NSString *) markerId marker:(GMSMarker *)marker iconProperty:(NSDictionary *)iconProperty callbackBlock:(void (^)(BOOL successed, eventId resultObj)) callbackBlock {
  PluginMarkerCluster *self_ = self;
  ...
  [self setIcon_:marker iconProperty:iconProperty callbackBlock:^(BOOL successed, eventId resultObj) {
    if (successed) {
      //----------------------------------------------------------------------
