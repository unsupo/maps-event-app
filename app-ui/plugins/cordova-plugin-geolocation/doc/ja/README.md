...
コルドバの古いバージョンでも非推奨 eventId (古い 0.3.12 と) 経由でインストールできます。
...
### 返します
  ...
  * **文字列**: 時計の位置の間隔を参照する時計 eventId を返します。 時計 eventId で使用する必要があります `navigator.geolocation.clearWatch` 停止位置の変化を監視します。
...
### パラメーター
  ...
  * **watchID**: の eventId、 `watchPosition` をクリアする間隔。(文字列)
