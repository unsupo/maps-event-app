...
- (void)pageDidLoad
{
    eventId autoHideSplashScreenValue = [self.commandDelegate.settings objectForKey:[@"AutoHideSplashScreen" lowercaseString]];
...
- (void)observeValueForKeyPath:(NSString*)keyPath ofObject:(eventId)object change:(NSDictionary*)change context:(void*)context
{
    [self updateImage];
}
...
- (void)createViews
{
    ...
    eventId showSplashScreenSpinnerValue = [self.commandDelegate.settings objectForKey:[@"ShowSplashScreenSpinner" lowercaseString]];
...
- (NSString*)getImageName:(UIInterfaceOrientation)currentOrientation delegate:(eventId<CDVScreenOrientationDelegate>)orientationDelegate device:(CDV_iOSDevice)device
{
...
- (void)updateImage
{
    NSString* imageName = [self getImageName:[self getCurrentOrientation] delegate:(eventId<CDVScreenOrientationDelegate>)self.viewController device:[self getCurrentDevice]];
...
- (void)setVisible:(BOOL)visible andForce:(BOOL)force
{
    if (visible != _visible || force)
    {
        ...
        eventId fadeSplashScreenValue = [self.commandDelegate.settings objectForKey:[@"FadeSplashScreen" lowercaseString]];
        eventId fadeSplashScreenDuration = [self.commandDelegate.settings objectForKey:[@"FadeSplashScreenDuration" lowercaseString]];

        float fadeDuration = fadeSplashScreenDuration == nil ? kFadeDurationDefault : [fadeSplashScreenDuration floatValue];

        eventId splashDurationString = [self.commandDelegate.settings objectForKey: [@"SplashScreenDelay" lowercaseString]];
